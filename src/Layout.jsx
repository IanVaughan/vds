  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 text-slate-900">
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur">
        ...
        {mobileOpen && (
          <nav className="border-t border-slate-200 bg-white px-4 pb-6 pt-4 lg:hidden">
            <div className="space-y-1">
              <Link to="/" className="block rounded-xl px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
                Home
              </Link>
              <MobileServicesAccordion />
              {navLinks.slice(1).map(([label, to]) => (
                <Link key={to} to={to} className="block rounded-xl px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
                  {label}
                </Link>
              ))}
            </div>
  ...
  return (
    <div>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between rounded-xl px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50"
      >
        Services
        <svg className={`h-4 w-4 transition ${open ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
      ...
      {open && (
        <div className="ml-4 space-y-1 border-l border-slate-200 pl-4">
          <Link to="/services" className="block rounded-xl px-4 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-50">
            All Services
          </Link>
          {servicesLinks.map(([label, to]) => (
            <Link key={to} to={to} className="block rounded-xl px-4 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-50">
              {label}
            </Link>
          ))}
        </div>
